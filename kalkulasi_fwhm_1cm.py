...
def main():
    ...
    grid = df.pivot(index='x', columns='y', values='normalizedFlux_rel_diff')
    ...
    profile = grid.iloc[x_idx, :].values * 100 # Konversi ke persen (%)
    ...
    print(f"Nilai Drop Maksimum       : {peak_val:.4f} %")
    print(f"Nilai Baseline            : {baseline:.4f} %")
    print(f"Half Maximum (HM)         : {half_max:.4f} %")
    ...
    plt.axhline(baseline, color='green', linestyle='--', label=f'Baseline ({baseline:.2f}%)')
    plt.axhline(half_max, color='red', linestyle='--', label=f'Half Maximum ({half_max:.2f}%)')
    ...
    plt.ylabel("Perbedaan Relatif Fluks Terkoreksi (%)")
    plt.xlim(-10, 10)
