...
"""
=============================================================================
RICHARDSON-LUCY DECONVOLUTION (ITERATIVE MAXIMUM LIKELIHOOD)
=============================================================================
Skrip ini mengimplementasikan dekonvolusi citra/sinyal 1D berbasis algoritma
iteratif Richardson-Lucy (Maximum Likelihood Expectation Maximization):

               S^(k+1)(x) = S^(k)(x) * [ H(-x) * ( I(x) / (H(x) * S^(k)(x)) ) ]

di mana:
  - I(x)     : Sinyal observasi terdegradasi/blur
  - H(x)     : Point Spread Function (PSF) sistem detektor
  - H(-x)    : Kernel adjoint (flipped PSF)
  - S^(k)(x) : Estimasi sinyal asli pada iterasi ke-k
  - *        : Operator konvolusi

Hasil dekonvolusi diukur menggunakan lebar FWHM (Full Width at Half Maximum)
dan dibandingkan langsung dengan dimensi keretakan aslinya (Ground Truth).
=============================================================================
"""
...
# -----------------------------------------------------------------------------
# METODE 1: Richardson-Lucy Deconvolution (Iterative Maximum Likelihood)
# -----------------------------------------------------------------------------
def apply_richardson_lucy_deconv(profile_1d, psf_kernel, num_iterations=15, eps=1e-12):
    """
    Dekonvolusi Iteratif Richardson-Lucy (Maximum Likelihood):
    S^(k+1)(x) = S^(k)(x) * [ H(-x) * ( I(x) / (H(x) * S^(k)(x)) ) ]
    """
    I = np.array(profile_1d, dtype=float)
    
    # Richardson-Lucy diturunkan untuk distribusi Poisson (non-negatif).
    # Geser baseline bila terdapat fluktuasi noise di bawah nol.
    min_val = np.min(I)
    if min_val < 0:
        I_shifted = I - min_val + eps
    else:
        I_shifted = I + eps

    # Normalisasi PSF agar luas area = 1 (konversi energi/fluks)
    H = np.array(psf_kernel, dtype=float)
    H = H / np.sum(H)
    H_rev = H[::-1]  # H(-x)

    # Inisialisasi awal S^(0)(x) dengan sinyal observasi
    S = np.copy(I_shifted)

    for k in range(num_iterations):
        # 1. Proyeksi maju: H(x) * S^(k)(x)
        reconv = np.convolve(S, H, mode='same')
        reconv = np.maximum(reconv, eps)

        # 2. Rasio perbandingan: I(x) / [H(x) * S^(k)(x)]
        ratio = I_shifted / reconv

        # 3. Proyeksi balik dengan kernel terbalik: H(-x) * ratio
        correction = np.convolve(ratio, H_rev, mode='same')

        # 4. Pembaruan multiplikatif
        S = S * correction

    # Kembalikan offset baseline
    if min_val < 0:
        S = S + min_val - eps
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Analisis Richardson-Lucy Deconvolution & FWHM Keretakan GCB")
    parser.add_argument("--iterations", type=int, default=15, help="Jumlah iterasi Richardson-Lucy, default=15")
    ...
    print("=" * 105)
    print(" ANALISIS RICHARDSON-LUCY DECONVOLUTION & PENGUKURAN FWHM KERETAKAN ".center(105, "="))
    print("=" * 105)
    print(f"Direktori Data           : {base_dir}")
    print(f"Jumlah Dataset Ditemukan : {len(file_list)}")
    print(f"Model PSF Detektor       : Gaussian (sigma = {sigma_psf:.2f} cm)")
    print(f"Jumlah Iterasi R-L       : {num_iter} iterasi")
    print("=" * 105)
    ...
    for file_path, file_name, actual_w in file_list:
        ...
        # 3. Lakukan Richardson-Lucy Deconvolution
        y_deconv = apply_richardson_lucy_deconv(y_raw, psf_kernel, num_iterations=num_iter)
        ...
        results.append({
            ...
            'RelErr_RL_pct': rel_err_dec,
            'Deriv_Raw_cm': w_deriv_raw,
            'Deriv_RL_cm': w_deriv_dec
        })

    res_df = pd.DataFrame(results)

    # -------------------------------------------------------------------------
    # CETAK TABEL PERBANDINGAN FWHM RAW VS RICHARDSON-LUCY KE TERMINAL
    # -------------------------------------------------------------------------
    print(f"{'No':<4} {'Retakan':<12} {'Asli (cm)':<10} {'FWHM Raw':<11} {'FWHM R-L':<11} {'Err Raw':<10} {'Err R-L':<10} {'%Err Raw':<10} {'%Err R-L':<10}")
    print("-" * 105)
    for idx, row in res_df.iterrows():
        sign_raw = f"{row['Err_Raw_cm']:+.3f}"
        sign_dec = f"{row['Err_RL_cm']:+.3f}"
        print(f"{idx+1:<4} {row['Tag']:<12} {row['Actual_cm']:<10.2f} {row['FWHM_Raw_cm']:<11.3f} {row['FWHM_RL_cm']:<11.3f} {sign_raw:<10} {sign_dec:<10} {row['RelErr_Raw_pct']:<+10.1f} {row['RelErr_RL_pct']:<+10.1f}")
    print("=" * 105)

    # -------------------------------------------------------------------------
    # STATISTIK RINGKASAN PERFORMA
    # -------------------------------------------------------------------------
    valid_df = res_df.dropna(subset=['Actual_cm', 'FWHM_Raw_cm', 'FWHM_RL_cm'])
    
    mae_raw = np.mean(np.abs(valid_df['Err_Raw_cm']))
    mae_dec = np.mean(np.abs(valid_df['Err_RL_cm']))
    rmse_raw = np.sqrt(np.mean(valid_df['Err_Raw_cm']**2))
    rmse_dec = np.sqrt(np.mean(valid_df['Err_RL_cm']**2))
    ...
    mae_dec_small = np.mean(np.abs(small_df['Err_RL_cm']))

    print("\n" + "=" * 70)
    print(" RINGKASAN AKURASI & EFEKTIVITAS RICHARDSON-LUCY ".center(70, "="))
    print("=" * 70)
    print(f"1. RETAKAN KECIL (<= 1.0 cm, Area Dominasi Blurring PSF):")
    print(f"   * MAE FWHM Raw               : {mae_raw_small:.4f} cm")
    print(f"   * MAE FWHM Richardson-Lucy   : {mae_dec_small:.4f} cm")
    ...
    print(f"   * Mean Absolute Error (R-L)  : {mae_dec:.4f} cm")
    print(f"   * RMSE Raw vs Richardson-Lucy: {rmse_raw:.4f} cm vs {rmse_dec:.4f} cm")
    print("=" * 70)
    ...
    dec_array = valid_df['FWHM_RL_cm'].values
    ...
    print(f"Persamaan Kalibrasi FWHM R-L        : d = ({coeffs_dec[0]:.4f})*W^2 + ({coeffs_dec[1]:.4f})*W + ({coeffs_dec[2]:.4f})")
    print(f"MAE Setelah Kalibrasi R-L           : {mae_cal_dec:.4f} cm")
    print("=" * 80)

    # Simpan hasil ke CSV
    out_csv = os.path.join(base_dir, "richardson_lucy_fwhm_comparison.csv")
    res_df['Calibrated_Raw_cm'] = pred_cal_raw
    res_df['Calibrated_RL_cm'] = pred_cal_dec
